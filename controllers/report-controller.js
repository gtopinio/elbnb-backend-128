...
exports.generateReport = (pool) => (req, res) => {
    ...
    //       return res.send({ success: false });
    //     } else {
    //       connection.query('SELECT accommodation.*, MAX(room.ROOM_PRICE) AS max_price, user.USER_USERNAME, AVG(review.REVIEW_RATING) AS rating ' +
    //                        'FROM user INNER JOIN accommodation ON user.USER_ID = accommodation.ACCOMMODATION_OWNER_ID ' + 
    //                        'INNER JOIN review ON accommodation.ACCOMMODATION_ID = review.ACCOMMODATION_ID ' +
    //                        'LEFT JOIN room ON accommodation.ACCOMMODATION_ID = room.ACCOMMODATION_ID ' + 
    //                        'WHERE accommodation.ACCOMMODATION_ISARCHIVED = false AND room.ROOM_ISARCHIVED = false ' + 
    //                        'GROUP BY accommodation.ACCOMMODATION_ID ' + 
    //                        'ORDER BY accommodation.ACCOMMODATION_NAME', 
    //                        (err, results) => {
    //         if (err) {
    //           console.log("Error: " + err);
    //           return res.send({ success: false });
